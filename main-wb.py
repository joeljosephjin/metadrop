...
net_cent = net['cent']
net_acc = net['acc']
net_acc_mean = tf.reduce_mean(input_tensor=net['acc'])
...
tnet_cent = tnet['cent']
tnet_acc = tnet['acc']
tnet_acc_mean = tf.reduce_mean(input_tensor=tnet['acc'])
...
def meta_train():
  ...
  lr = tf.convert_to_tensor(value=args.meta_lr)

  optim = tf.compat.v1.train.AdamOptimizer(lr)
...
def meta_test():
  ...
  config.gpu_options.allow_growth = True
  sess = tf.compat.v1.Session(config=config)
  saver = tf.compat.v1.train.Saver(tnet_weights)
